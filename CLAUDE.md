...
When adding or renaming a skill, also update the skills table in the root `README.md` and its plugin entry in `.claude-plugin/marketplace.json`. `docs/how-to-use.md` documents the generic install/authoring flow.

## Distribution

The repo is a Claude Code plugin marketplace: `.claude-plugin/marketplace.json` declares one plugin per skill, each with `source: "./skills/<name>"` (the `SKILL.md` sits at the plugin root), so users install skills individually: `/plugin marketplace add stefanoleone/claude-code-skills`, then `/plugin install <skill-name>@claude-code-skills`. The plugins deliberately declare no `version`: versioning rides on git commit SHAs, so every commit to `main` reaches users on `/plugin marketplace update` — do not add a `version` field without adopting a release process.
